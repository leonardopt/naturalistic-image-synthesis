"""
Diagnostic: alternative SDXL reproducibility test (variant of diagnostic_generation.py).

Loads the SDXL base + refiner + LoRA stack once and generates two images from
the same pre-built generator and noise latent, passing the generator object
directly to setup_and_generate_image() rather than recreating it per call.
Both outputs should be identical; differences indicate a generator-state issue.
"""
import os
...
def setup_and_generate_image(pipe, refiner, generator, init_latent, prompt, negative_prompt, output_file):
    # Generate image
...
if __name__ == "__main__":
    ...
    # Generate images
    setup_and_generate_image(pipe, refiner, generator, init_latent, prompt, negative_prompt, output_file1)
    setup_and_generate_image(pipe, refiner, generator, init_latent, prompt, negative_prompt, output_file2)
