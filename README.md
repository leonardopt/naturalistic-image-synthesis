...
**(S1)** A text prompt is designed for each of the 108 object-scenes from 6 categories (e.g., *"award-winning marine photo of a colorful fish in a coral reef, centered in the scene, vibrant underwater scene, high detail"*).
...
**(S3)** Pairwise [LPIPS](https://arxiv.org/abs/1801.03924) (Learned Perceptual Image Patch Similarity) distances are computed among all 60 exemplars. A pair of images — an *anchor* (most typical) and a *guide* (most typical near-neighbour) — is selected as the endpoints for interpolation.
...
In the codebase, these conceptual steps are implemented through a sequence of
scripts rather than a strict one-script-per-step mapping:

- **(S1) Design prompts and stimulus structure**
  `pipe_01_generate_design.py`

- **(S2) Generate anchor exemplars**
  `pipe_02a_generate_images.py`
  `pipe_02b_exclude_images.py` (optional manual exclusion step)

- **(S3) Compute anchor similarities and select endpoint pairs**
  `pipe_03_compute_similarities.py`
  `pipe_04a_select_pairs.py`
  `pipe_04b_collect_selected_pairs.py` / `pipe_04c_change_pair.py` / `pipe_04d_collect_final_pairs.py`
  manual review helpers around pair selection

- **(S4) Generate interpolations**
  `pipe_05_generate_interpolations.py`

- **(S5) Score interpolations and select final frames**
...
- A LoRA weight file ([`xl_more_art-full_v1.safetensors`](https://huggingface.co/ffxvs/lora-effects-xl/blob/main/xl_more_art-full_v1.safetensors)) placed in a `LoRAs/` directory at the repository root
